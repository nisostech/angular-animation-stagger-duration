! function (name, definition) {
    ...
    cssInjector.getStylesheet = function () {
        var self = this,
            si, sl;
        ...
        if (!self.element) {
            ...
            self.element.setAttribute('id', 'cssInjector');
    ...
    cssInjector.inject = function (selectors, css, options) {
...
function animation() {
    return function (scope, elm, attr) {
        var className = attr.uniquekey.toString()
            //elm.css({"-webkit-animation-name": "slideInRight",
            //           "animation-name": "slideInRight"})
            //console.log(elm.attr('class').split(" "))
            // get enter animation
        var enterConfig = scope.$eval(attr.enterAnimation);
        var leaveConfig = scope.$eval(attr.leaveAnimation)

        elm.addClass("animated")
        elm.addClass(className)
        var staggerDuration = attr.stagger ? attr.stagger : 0
        var duration = attr.duration ? attr.duration : 0
        var ngEnterClass = "." + className + ".ng-enter"
        var ngLeaveClass = "." + className + ".ng-leave"
        var ngEnterStaggerClass = "." + className + ".ng-enter-stagger"
        var ngLeaveStaggerClass = "." + className + ".ng-leave-stagger"
            //ng-enter class
        cssInjector.inject(ngEnterClass.toString(), {
            'transition': duration + "s",
            "-webkit-animation-name": "slideInRight",
            "animation-name": "slideInRight"
        });
        //leave class
        cssInjector.inject(ngLeaveClass.toString(), {
        ...
        //enter stager
        cssInjector.inject(ngEnterStaggerClass.toString(), {
            "transition-delay": staggerDuration + "s",
            "animation-duration": "0s"
        });
        //leaver stagger
        cssInjector.inject(ngLeaveStaggerClass.toString(), {
            "animation-delay": "1s",
...
angular.module('angular-animation', ['ngAnimate']).directive('animation', animation)